...
enum AVPixelFormat get_cuda_format(AVCodecContext *ctx, const enum AVPixelFormat *pix_fmts)
{
...
int open_input_file(const char *filename, AVFormatContext **ifmt_ctx, int &audio_stream, int &video_stream, AVCodecContext **audio_decoder_ctx, AVCodecContext **video_decoder_ctx, AVBufferRef **hw_device_ctx)
{
    int ret;
    ...
    /*获取流索引*/
    /*视频流*/
    ret = av_find_best_stream(*ifmt_ctx, AVMEDIA_TYPE_VIDEO, -1, -1, &video_decoder, 0);
    if (ret < 0)
    {
        char errStr[256] = {0};
        av_strerror(ret, errStr, sizeof(errStr));
    ...
    *video_decoder_ctx = avcodec_alloc_context3(video_decoder);
    ...
    (*video_decoder_ctx)->get_format = get_cuda_format;
    ...
    /*音频流,音频不存在就跳过*/
    ret = av_find_best_stream(*ifmt_ctx, AVMEDIA_TYPE_AUDIO, -1, -1, &audio_decoder, 0);
    if (ret < 0)
    {
        audio_stream = -1;
        char errStr[256] = {0};
    ...
    *audio_decoder_ctx = avcodec_alloc_context3(audio_decoder);
    ...
    ret = avcodec_open2(*audio_decoder_ctx, audio_decoder, NULL);
    if (ret < 0)
    {
        char errStr[256] = {0};
        av_strerror(ret, errStr, sizeof(errStr));
...
// const char *rtpUrl = "[select=v:f=rtp:ssrc=2222:payload_type=101]rtp://192.168.0.20:49948?rtcpport=42188";
#ifdef Debug
    const char *rtpUrl = "[select=a:f=rtp:ssrc=1111:payload_type=100]rtp://10.8.14.118:34157?rtcpport=34016|[select=v:f=rtp:ssrc=2222:payload_type=101]rtp://10.8.14.118:34239?rtcpport=34650";
#else
    char rtpUrl[512];
#endif
    ...
    ret = open_input_file(rtspUrl, &ifmt_ctx, in_audio_stream, in_video_stream, &audio_decoder_ctx, &video_decoder_ctx, &hw_device_ctx);
    if (ret < 0)
    {
        goto end;
    }
    ...
    if (in_audio_stream >= 0)
    {
        ...
        // audio_enc_codec = avcodec_find_encoder(AV_CODEC_ID_PCM_MULAW);
        if (!audio_enc_codec)
        {
    ...
    /*创建输出上下文*/
    // ret = (avformat_alloc_output_context2(&ofmt_ctx, nullptr, "flv", rtmpUrl));
    // ret = (avformat_alloc_output_context2(&ofmt_ctx, nullptr, "mp4", rtmpUrl));
